      ...
      try {
        ...
        const transformedPosts = data.data.map((insight: any) => ({
          ...
          image: insight.thumbnail?.url?.includes('http')
            ...
            : `${strapiUrl}${insight.thumbnail?.url || '/assets/images/blog/blog-1.png'}`,
...
export function BlogOld({ title }: { title?: string }) {
  return (
    <section>
      <div className={clsx('content-wrapper mx-auto')}>
        {/* Section Title */}
        ...
        >
          {blogPosts.map((post) => (
            <BlogCardOld key={post.id} post={post} />
          ))}
        </div>
