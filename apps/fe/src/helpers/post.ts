...
export const getPostOgImageUrl = (data: TPostOgImage) => {
  const getUrl = (aspectRatio?: TPostOgImage["aspectRatio"]) => {
    const params = aspectRatio ? getParams({ ...data, aspectRatio }) : getParams(data)
...
export const getPostStructuredData = ({ title, dateModified, datePublished, images }: { title: string; images: Array<string>; datePublished: string; dateModified: string }) =>
  JSON.stringify({
